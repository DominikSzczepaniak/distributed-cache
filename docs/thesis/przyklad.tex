...
\textbf{Raft} is a modern algorithm, introduced in 2014 by Diego Ongaro and John Ousterhout, that was designed to be easier to implement than Paxos and more understandable than it. It offers performance comparable to Multi-Paxos. It decomposes the problem of consensus into smaller, independent sub-problems of Leader election, Log Replication and Safety. Raft is widely used in many modern distributed systems such as CockroachDB[3], MongoDB[4], Neo4J[5], RabbitMQ[6]. 
...
To distribute the data between multiple server, the process called sharding is introduced [7].
...
Consistent hashing maps both data and servers onto a logical ring topology ($0$ to $2^{32}-1$). A key is assigned to the first server encountered moving clockwise on the ring. [11] %TODO hyperlink citation
